    ...
    value: function componentDidMount() {
      ...
      if (this.refs.toolbar) {
        this.setState({
          ...
          height: this.refs.editarea.refs.edit.offsetHeight
        });
        var self = this;
        if (typeof window !== "undefined") {
          window.onscroll = function (e) {
            ...
            if (-document.documentElement.getBoundingClientRect().top > self.state.top && -document.documentElement.getBoundingClientRect().top < self.state.height) {
              self.setState({
                style: {
                  position: "fixed",
                  top: "0",
